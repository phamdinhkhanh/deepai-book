...
get_ipython().run_cell_magic('script', 'echo skipping', "# Để cho nhanh thì mình sẽ training trên 100 skip_grams đầu tiên.\nfor epoch in range(1, 6):\n    loss = 0\n    for i, elem in enumerate(skip_grams[:100]):\n        pair_first_elem = np.array(list(zip(*elem[0]))[0], dtype='int32')\n        pair_second_elem = np.array(list(zip(*elem[0]))[1], dtype='int32')\n        labels = np.array(elem[1], dtype='int32')\n        X = [pair_first_elem, pair_second_elem]\n        Y = labels\n        if i % 500 == 0:\n            print('Processed {} (skip_first, skip_second, relevance) pairs'.format(i))\n        loss += model.train_on_batch(X,Y)  \n\n    print('Epoch:', epoch, 'Loss:', loss)")
...
get_ipython().run_cell_magic('script', 'echo skipping', '!sudo apt-get install tesseract-ocr')
...
get_ipython().run_cell_magic('script', 'echo skipping', 'import cv2\nfrom PIL import Image\nimport pytesseract\n\nimg_rgb = cv2.cvtColor(img_arr, cv2.COLOR_BGR2RGB)\nprint(image_to_string(img_rgb))')
...
get_ipython().run_cell_magic('script', 'echo skipping', '# install package reverse_geocoder\n!pip install reverse_geocoder')
...
get_ipython().run_cell_magic('script', 'echo skipping', '!pip install user_agents')
...
get_ipython().run_cell_magic('script', 'echo skipping', '!pip install mlxtend')
